...
class ItemProducto(MDCard):
    ...
    def on_kv_post(self, base_widget):
        self.opacity = 0
        self.elevation = 0
        anim = Animation(opacity=1, elevation=2, d=0.4, t='out_quad')
...
class PantallaInventario(MDScreen):
    ...
    def cargar_productos(self, filtro=""):
        ...
        # Crear widgets en batch para mejor rendimiento
        widgets = []
        ...
        # Agregar todos los widgets de una vez
        for widget in widgets:
            self.ids.lista_productos.add_widget(widget)
    ...
    def cargar_ventas(self):
        try:
            # Solo recargar si hay cambios significativos o primera carga
            ...
            # Agregar todos los widgets de una vez
            for widget in widgets:
                self.ids.lista_ventas.add_widget(widget)
            
            self.ids.label_total_ventas.text = f"${total_acumulado:,.2f}"
            
            # Forzar actualización del layout usando Clock
            def actualizar_layout(dt):
                self.ids.lista_ventas.height = self.ids.lista_ventas.minimum_height
            
            Clock.schedule_once(actualizar_layout, 0.1)
    ...
    def exportar_ventas_csv(self):
        ...
        if not datos:
            ...
            return
        self._exportar_csv("Exportar ventas CSV", "reporte_ventas.csv", ["N", "Producto", "Categoría", "Cantidad", "Total ($)", "Fecha", "Método Pago"], datos)
    ...
    def exportar_ventas_excel(self):
        ...
        if not datos:
            self.mostrar_alerta("No hay ventas para exportar.", tipo="error")
            return
        self._exportar_csv("Exportar ventas Excel", "reporte_ventas.csv", ["N", "Producto", "Categoría", "Cantidad", "Total ($)", "Fecha", "Método Pago"], datos, codificacion="utf-8-sig")
